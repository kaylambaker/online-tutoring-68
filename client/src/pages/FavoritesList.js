    ...
    axios.get(`http://localhost:8800/students/favorites_list/${StudentID}`)
      .then(async (response) => {
        if (response.data && response.data.length > 0) {
          ...
          // Fetch student and tutor details for each favorite
          for (const favorite of response.data) {
            const studentResponse = await axios.get(`/students/${StudentID}`);
            const tutorResponse = await axios.get(`/tutors/${favorite.TutorID}`);
            ...
            const tutorDetails = {
              ...
              tutorName: tutorResponse.data.FirstName + ' ' + tutorResponse.data.LastName,
              studentName: studentResponse.data.FirstName + ' ' + studentResponse.data.LastName,
            };
  ...
  return (
    <div className="container">
      ...
      {favorites.map((favorite) => (
        <div key={favorite.id}>
          <p>Tutor Name: {favorite.tutorName}</p>
          <p>Student Name: {favorite.studentName}</p>
